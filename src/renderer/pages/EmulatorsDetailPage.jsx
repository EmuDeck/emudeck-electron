...
function EmulatorsDetailPage() {
  ...
  const { state, setState, stateCurrentConfigs, setStateCurrentConfigs } = useContext(GlobalContext);
  const { installEmus, installFrontends, mode, system, yuzuEAtoken, emulatorAlternative } = state;
  ...
  const { disabledNext, disabledBack, emulatorSelected, modal, updates, newDesiredVersions, lastSelected } = statePage;
  ...
  const yuzuEAsetToken = (data) => {
    ...
    data.target.value === "" ? (yuzuEAtokenValue = null) : (yuzuEAtokenValue = data.target.value);
  ...
  const yuzuEAaskToken = () => {
    const modalData = {
      ...
      body: (
        <>
          <p>Enter your Yuzu Early Access Token to automatically download and update Yuzu Early Access.</p>
          <p>You can get this from your Yuzu Patreon.</p>
          <p>https://yuzu-emu.org/help/early-access/</p>
          <p>Once you have entered your token in this window it will be saved to ~/emudeck/yuzu-ea-token.txt</p>
          <div className="form">
            <FormInputSimple
      ...
      footer: (
        <BtnGroup>
          <BtnSimple css="btn-simple--2" type="button" aria="Close Modal" onClick={() => closeModal()}>
            Close
          </BtnSimple>
          <BtnSimple css="btn-simple--1" type="button" aria="Add Token" onClick={() => yuzuEAaddToken()}>
            Next
          </BtnSimple>
        </BtnGroup>
      ),
  ...
  const yuzuEAaddToken = () => {
    ...
    ipcChannel.sendMessage("emudeck-legacy", [`YuzuEA_addToken|||YuzuEA_addToken "${yuzuEAtoken}"`]);
    let modalHeader;
    let modalBody;
    let modalFooter;
    ipcChannel.once("YuzuEA_addToken", (message) => {
      ...
      switch (true) {
        ...
        case response.includes("fail"):
          ...
          modalBody = "There was an issue installing Yuzu Early Access, please try again.";
          break;
        case /true|OK/.test(response):
          modalHeader = <span className="h4">Yuzu Early Access Success!</span>;
          modalBody = (
            <p>
              Yuzu Early Access has been installed, you can play games as always. EmuDeck will detect you have Yuzu EA
              and use that instead.You don't need to do setup anything else.
            </p>
          );
          break;
        default:
    ...
    ipcChannel.once(`${code}_install`, (message) => {
      ...
      if (/true|OK/.test(status)) {
        const modalData = {
          ...
          body: (
            <p>
              {name} has been installed, now you can play games from {name} using EmulationStation-DE or adding them to
              your Steam Library using Steam Rom Manager
            </p>
          ),
          css: "emumodal--xs",
        };
        ...
        if (emulator === "esde" || emulator === "pegasus" || emulator === "srm") {
          if (emulator === "srm") {
            emulator = "steam";
          }
  ...
  const installEmu = (emulator, code, name) => {
    ...
    ipcChannel.once(`${code}_install`, (message) => {
      ...
      ipcChannel.sendMessage("emudeck", [`${code}_is_installed|||${code}_is_installed`]);
      ipcChannel.once(`${code}_is_installed`, (message) => {
        ...
        if (/true|OK/.test(status)) {
          ...
          const modalData = {
            ...
            body: (
              <p>
                {name} has been installed, now you can play games from {name} using EmulationStation-DE or adding them
                to your Steam Library using Steam Rom Manager
              </p>
            ),
            css: "emumodal--xs",
          };
          ...
          if (
            ...
            const modalData = {
              ...
              body: (
                <p>
                  If you want to use this new Standalone emulator in Steam you need to go to Steam Rom Manager and pick
                  the proper parser.
                </p>
              ),
              css: "emumodal--xs",
  ...
  const uninstallEmu = (emulator, code, alternative = false) => {
    ...
    if (alternative) {
      ipcChannel.sendMessage("emudeck-legacy", [`${code}_uninstall|||${code}_uninstall_alt`]);
    } else {
      ipcChannel.sendMessage("emudeck", [`${code}_uninstall|||${code}_uninstall`]);
    }
      ...
      ipcChannel.once(`${code}_is_installed`, (message) => {
        ...
        if (status.includes("KO")) {
          const modalData = {
            ...
            body: (
              <p>
                {code} has been uninstalled, you will need to delete your entries from Steam using Steam Rom Manager and
                manually delete your saved games in Emulation/saves/{code}
              </p>
            ),
            css: "emumodal--xs",
          };
    ...
    ipcChannel.once(`${code}_init`, (status) => {
      ...
      if (/true|OK/.test(status)) {
        const modalData = {
          ...
          body: <p>{name}'s configuration was updated with our latest improvements, optimizations and bug fixes!</p>,
          css: "emumodal--xs",
        };
  ...
  const onClickMigrate = (code) => {
    const modalData = {
      active: true,
      header: <span className="h4">Migrate {code} from Flatpak to AppImage</span>,
      body: (
        <>
          <p>
            It's migration time! To keep things simple, the version of RPCS3 you are using is unofficially supported by
            the primary developers of RPCS3. This migration will download the RPCS3 AppImage, which will also include
            frequent updates and performance improvements.
          </p>
          <p>
            EmuDeck will migrate your saves and your configurations (yes your per-game configurations too), so you can
            continue playing right away.
          </p>
        </>
      ),
      footer: (
        <BtnGroup>
          ...
          <BtnSimple css="btn-simple--1" type="button" aria="Add Token" onClick={() => doMigration(code)}>
            Migrate
          </BtnSimple>
        </BtnGroup>
      ),
  ...
  const installOptional = () => {
    ...
    ipcChannel.sendMessage("emudeck-legacy", [`API_optional_parsers|||API_optional_parsers`]);
  ...
  useEffect(() => {
    const ogStateAlternative = JSON.parse(localStorage.getItem("ogStateAlternative"));
      ...
      ipcChannel.once(`parsersUpdate`, (message) => {
        ...
        if (/true|OK/.test(status)) {
          modalData = {
            ...
            footer: (
              <BtnSimple css="btn-simple--1" type="button" onClick={() => navigate("/welcome")}>
                Close
              </BtnSimple>
            ),
            css: "emumodal--xs",
