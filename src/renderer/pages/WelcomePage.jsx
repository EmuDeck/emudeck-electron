...
function WelcomePage() {
  ...
  const { system, systemName, mode, second, storagePath, gamemode, branch } = state;
  ...
  const { disabledNext, disabledBack, updates, modal, dom, news, game_of_the_month } = statePage;

  const navigate = useNavigate();

  const newsWS = useFetchCond(`https://token.emudeck.com/news.php?branch=${branch}`);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    ipcChannel.once("check-versions", (repoVersions) => {
      ...
      const currentVersions = JSON.parse(localStorage.getItem("current_versions"));
  ...
  return (
    <Wrapper aside={second === true}>
      ...
      {systemName !== "ERROR" && second === false && <Welcome onClick={selectMode} />}
        ...
        <>
          ...
          <Main css="news">
            ...
            <span className="h2">
              ...
              <a href="https://retrohandhelds.gg" target="blank" style={{ display: "inline" }}>
                <img
