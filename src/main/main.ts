...
ipcMain.on('pull', async (event, branch) => {
  ...
  let bashCommand = `cd ~/.config/EmuDeck/backend &&  script ~/.config/EmuDeck/msg.log  -c 'git reset --hard' &&  script ~/.config/EmuDeck/msg.log -c  'git clean -fd' && git checkout ${branchGIT} && script ~/.config/EmuDeck/msg.log -c 'git pull' && . ~/.config/EmuDeck/backend/functions/all.sh && appImageInit`;
