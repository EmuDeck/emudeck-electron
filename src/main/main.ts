...
ipcMain.on('pull', async (event, branch) => {
  ...
  let preCommand = `cd ${appDataPath}/backend && git reset --hard && git clean -fd && git checkout ${branchGIT} && git pull`;

  preCommand = `cd ${appDataPath}/backend && git checkout ${branchGIT} && git pull`;
